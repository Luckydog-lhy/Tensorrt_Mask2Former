...
class DefaultPredictor:
    ...
    def trace_model(self, original_image,TRT=True):
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            # Apply pre-processing to image.
            if self.input_format == "RGB":#/mask2former/modeling/transformer_decoder/mask2former_transformer_decoder.py
                # whether the model expects BGR inputs or RGB
                original_image = original_image[:, :, ::-1]
            height, width = original_image.shape[:2]
            image = self.aug.get_transform(original_image).apply_image(original_image)
            ...
            image = image.tensor
            backbone_torchscrpit = torch.jit.freeze(torch.jit.trace(self.model.backbone, image,check_trace=True) )
            ...
            class sem_seg_encoder(nn.Module):
                def __init__(self,sem_seg_head):
                    super(sem_seg_encoder, self).__init__()
                    self.sem_seg_encoder = sem_seg_head
                    self.sem_seg_encoder.only_encoder = True
                def forward(self,feature0,feature1,feature2,feature3,mask_flatten,reference_points):
                    features_pad = [feature0,feature1,feature2,feature3]
                    level_start_index = torch.as_tensor([0, 1050, 5250], device=features_pad[0].device)
                    spatial_shapes = []
                    for i in [0,1,2]:
                        h = 25 * math.pow(2, i)
                        w = 42 * math.pow(2, i)
                ...
                def prepare_inputs(self,feature0,feature1,feature2,feature3):
                    ...
                    reference_points = self.get_reference_points(spatial_shapes, valid_ratios, features[0].device)
                    ...
                    for i in [0,1,2]:
                        ...
                        spatial_shape = (h, w),
                        spatial_shapes.append(spatial_shape)
                    spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=features_pad[0].device).squeeze()
            ...
            class sem_seg_decoder(nn.Module):
                def __init__(self,sem_seg_head):
                    super(sem_seg_decoder, self).__init__()
                    self.sem_seg_decoder = sem_seg_head.predictor

                def forward(self,multi_scale_features0,multi_scale_features1,multi_scale_features2, mask_features):
                    multi_scale_features = [multi_scale_features0,multi_scale_features1,multi_scale_features2]
                    return self.sem_seg_decoder(multi_scale_features, mask_features)

            ##============================  encoder trace ===============================##
            print("=================== trace sem_seg_encoder ======================")
            sem_seg_encoder = sem_seg_encoder(self.model.sem_seg_head)
            sem_seg_encoder.eval()
            sem_seg_encoder_inputs = sem_seg_encoder.prepare_inputs(*features)
            sem_seg_encoder_torchscrpit = torch.jit.freeze(
            ...
            sem_seg_decoder = sem_seg_decoder(self.model.sem_seg_head)
            ...
            if TRT:
                sem_seg_decoder_torchscrpit = torch_tensorrt.compile(sem_seg_decoder_torchscrpit,
                    inputs=[
                        ...
                        torch_tensorrt.Input(
                            min_shape=[1, 256, 50, 84],opt_shape=[1, 256, 50, 84],max_shape=[1, 256, 50, 84],      dtype=torch.float32
                        ),
                        torch_tensorrt.Input(
                            min_shape=[1, 256, 100, 168],opt_shape=[1, 256, 100, 168],max_shape=[1, 256, 100, 168],dtype=torch.float32
                        ),
                        torch_tensorrt.Input(
                            min_shape=[1, 256, 200, 336],opt_shape=[1, 256, 200, 336],max_shape=[1, 256, 200, 336],dtype=torch.float32
                        ),
