...
auto squeeze_registrations TORCHTRT_UNUSED = RegisterNodeConversionPatterns().pattern(
    {"aten::squeeze.dim(Tensor(a) self, int dim) -> (Tensor(a))", //aten::squeeze.dim(Tensor(a) self, int dim) -> (Tensor(a))  aten::squeeze(Tensor(a) self) -> (Tensor(a))
     [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
       ...
       return true;
            ...
            {"aten::squeeze(Tensor(a) self) -> (Tensor(a))", //aten::squeeze.dim(Tensor(a) self, int dim) -> (Tensor(a))  aten::squeeze(Tensor(a) self) -> (Tensor(a))
             [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                 auto self = args[0].ITensorOrFreeze(ctx);
                 auto dim = 0;
