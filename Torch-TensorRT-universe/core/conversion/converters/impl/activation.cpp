        ...
        .pattern(
            {"aten::prelu(Tensor self, Tensor weight) -> (Tensor)",
             [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
               ...
               std::vector<int64_t> InputArraysShape = slopes.sizes().vec();

//               for (int ii = 0;ii< original_shape.nbDims;ii++){
//
//               }
//                 in->getAllowedFormats();
               if(InputArraysShape.size()!=original_shape.nbDims and InputArraysShape.size()==1){
                   std::vector<int64_t> slopes_new_shape = {1,InputArraysShape[0]};
                   for(int ii=0;ii<original_shape.nbDims -2;ii++){
                       slopes_new_shape.push_back(1);
                   }
