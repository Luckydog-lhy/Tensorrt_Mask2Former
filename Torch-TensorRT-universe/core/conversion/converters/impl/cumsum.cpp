...
auto einsum_registrations TORCHTRT_UNUSED = RegisterNodeConversionPatterns().pattern(
        {"aten::einsum(str equation, Tensor[] tensors) -> (Tensor)",
         [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
         std::string equation = args[0].unwrapTo<std::string>();
         auto inputs = args[1].IValue()->toListRef();
         ...
         einsum_layer->setEquation(equation.c_str() );
         auto cat_out = ctx->AssociateValueAndTensor(n->outputs()[0], einsum_layer->getOutput(0));
