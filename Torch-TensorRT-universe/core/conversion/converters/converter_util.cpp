...
nvinfer1::ITensor* castITensor(ConversionCtx* ctx, nvinfer1::ITensor* tensor, nvinfer1::DataType dtype) {
    ...
    LOG_DEBUG(ctx->logger, "Casting ITensor " << tensor_id.str() << " from " << tensor->getType() << " to " << casted_tensor->getType() );

    std::stringstream ss;
    ss << "[Cast["<<cast_id<<"] ITensor " << tensor_id.str() << " from " << tensor->getType() << " to " << dtype << "]";
...
nvinfer1::ILayer* add_elementwise(
  ...
  if (self->getType() != other->getType() ) {
     LOG_DEBUG(ctx->logger,"when call add_elementwise,cast the same dtype because it is not in same dtype");
...
//  nvinfer1::DataType post_freeze_cast_type = nvinfer1::DataType::kFLOAT;
  nvinfer1::DataType post_freeze_cast_type;
  auto t_dtype = t.scalar_type();
  switch (t_dtype)
  {
//      case at::kLong:
//          post_freeze_cast_type =  nvinfer1::DataType::kINT32;break;
//      case at::kDouble:
//          post_freeze_cast_type =  nvinfer1::DataType::kFLOAT; break;
//      case at::kFloat:
//          post_freeze_cast_type =  nvinfer1::DataType::kFLOAT; break;
      case at::kHalf:
          post_freeze_cast_type =  nvinfer1::DataType::kHALF; break;
//      case at::kInt:
//          post_freeze_cast_type =  nvinfer1::DataType::kINT32; break;
