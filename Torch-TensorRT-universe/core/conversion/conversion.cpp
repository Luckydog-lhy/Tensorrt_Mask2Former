      ...
      if (ctx->evaluated_value_map.find(out) != ctx->evaluated_value_map.end()) {
        ...
        if (out_ivalue.isCustomClass()) {
          ...
          TORCHTRT_THROW_ERROR("Tuple type. Only a single tensor or a TensorList type is supported.");
            ...
            std::vector<at::Tensor> OutputArrays;
            torch::List<torch::jit::IValue>    aa= out_ivalue.toList();
            int out_put_nums = 0;
            for (torch::jit::IValue a: aa ) {
                std::cout<<a<<std::endl;
                auto output_container = a.toCustomClass<TensorContainer>();
                nvinfer1::ITensor* out_tensor = output_container.get()->tensor();
                std::string name = std::string("output_") + std::to_string(ctx->num_outputs)+ std::string("_sub_") + std::to_string(ctx->num_outputs);
                out_tensor->setName(name.c_str());
                ctx->net->markOutput(*out_tensor);
                ctx->num_outputs++;
//                const torch::jit::IValue* a_p = &a;
////                auto out_ivalue = ctx->evaluated_value_map[a_p];
////                OutputArrays.emplace_back(a.toTensor() );
            }
...
std::unordered_map<c10::OperatorName, std::string> GetUnsupportedOpsInBlock(const torch::jit::Block* b) {
  ...
  for (const auto n : b->nodes()) {
    if (n->kind() != torch::jit::prim::Loop && n->kind() != torch::jit::prim::If && !OpSupported(n)) {
      ...
      std::cout<<schema->operator_name()<<std::endl;
        OpSupported(n);
