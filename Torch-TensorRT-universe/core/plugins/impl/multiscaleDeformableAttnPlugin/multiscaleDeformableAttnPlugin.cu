...
{
    TORCHTRT_CHECK((nbInputs == 5),"nbInputs not equal 5");
    TORCHTRT_CHECK((nbOutputs == 1),"nbOutputs not equal 1");
    ...
    nvinfer1::DynamicPluginTensorDesc const* outputs, int32_t nbOutputs) PLUGIN_NOEXCEPT
{
    ...
    TORCHTRT_CHECK(inputs[0].desc.dims.nbDims==4,"value nbDims not equal 4");
    TORCHTRT_CHECK(inputs[1].desc.dims.nbDims==2,"spatial_shape nbDims not equal 2");
    TORCHTRT_CHECK(inputs[2].desc.dims.nbDims==1,"level_start_index nbDims not equal 1");
    TORCHTRT_CHECK(inputs[3].desc.dims.nbDims==6,"sampling_location nbDims not equal 6");
    TORCHTRT_CHECK(inputs[4].desc.dims.nbDims==5,"atten_weight nbDims not equal 5");
    
    // Check M dimensions consistency
    TORCHTRT_CHECK(inputs[0].desc.dims.d[2] == inputs[3].desc.dims.d[2],"value d2 not equal sampling_location d2");
    TORCHTRT_CHECK(inputs[0].desc.dims.d[2] == inputs[4].desc.dims.d[2],"value d2 not equal atten_weight d2");

    // Check L dimensions consistency
    TORCHTRT_CHECK(inputs[1].desc.dims.d[0] == inputs[2].desc.dims.d[0],"spatial_shape d0 not equal level_start_index d3");
    TORCHTRT_CHECK(inputs[1].desc.dims.d[0] == inputs[3].desc.dims.d[3],"spatial_shape d0 not equal sampling_location d3");
    TORCHTRT_CHECK(inputs[1].desc.dims.d[0] == inputs[4].desc.dims.d[3],"spatial_shape d0 not equal atten_weight d3")

    // Check P dimensions consistency
    TORCHTRT_CHECK(inputs[3].desc.dims.d[4] == inputs[4].desc.dims.d[4],"sampling_location d4 not equal atten_weight d4");

    // Check Lq dimensions consistency
    TORCHTRT_CHECK(inputs[3].desc.dims.d[1] == inputs[4].desc.dims.d[1],"sampling_location d1 not equal atten_weight d1");
}
...
REGISTER_TORCHTRT_PLUGIN(MultiscaleDeformableAttnPluginCreator);
} // namespace impl
} // namespace plugins
} // namespace core
} // namespace torch_tensorrt