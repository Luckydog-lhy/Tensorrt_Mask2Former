...
std::vector<at::Tensor> execute_engine(std::vector<at::Tensor> inputs, c10::intrusive_ptr<TRTEngine> compiled_engine) {
  ...
  for (size_t i = 0; i < inputs.size(); i++) {
      std::string bind_name =  compiled_engine->cuda_engine->getBindingName(i);
      uint64_t pyt_idx = compiled_engine->in_string_binding_map[bind_name];
...
//        "Expected input tensors to have type " << expected_type << ", found type " << inputs[pyt_idx].dtype());
  ...
  for (size_t o = inputs.size(); o < (compiled_engine->num_io.first + compiled_engine->num_io.second); o++) {
    std::string bind_name =  compiled_engine->cuda_engine->getBindingName(o);
    uint64_t pyt_idx = compiled_engine->out_string_binding_map[bind_name];
...
TORCH_LIBRARY(tensorrt, m) {
  ...
  m.def("ms_deform_attn_tensorrt_forward", ms_deform_attn_tensorrt_forward);
}
