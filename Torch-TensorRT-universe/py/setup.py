...
def build_libtorchtrt_pre_cxx11_abi(develop=True, use_dist_dir=True, cxx11_abi=False):
    ...
    cmd = ["sh",os.path.join(dir_path,"build_cmake.sh")]
    status_code = subprocess.run(cmd)
    if status_code != 0:
        sys.exit(status_code)
...
def copy_libtorchtrt(multilinux=False):
    ...
    print("copying library into module")
    if multilinux:
        copyfile(dir_path + "../lib/libtorchTrt.so", dir_path + '/trtorch/lib/libtorchTrt.so')
    else:
        os.system("tar -xzf ../lib/libtorchtrt.tar.gz --strip-components=2 -C " + dir_path + "/torch_tensorrt")
...
ext_modules = [
    cpp_extension.CUDAExtension(
        ...
        library_dirs=[(dir_path + '/torch_tensorrt/lib/'), "/opt/conda/lib/python3.6/config-3.6m-x86_64-linux-gnu","/home/hongwei03/CLionProjects/Torch-TensorRT-universe/third_party/ms_deformable_atten/lib/"],
        libraries=["torchtrt"],
        include_dirs=[
